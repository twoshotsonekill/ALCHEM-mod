...
public class CreatorScreen extends HandledScreen<CreatorScreenHandler> {
    ...
    protected void drawBackground(DrawContext ctx, float delta, int mx, int my) {
        ...
        if (state == CreatorBlockEntity.STATE_PROCESSING) {
            drawProcessingEffect(ctx, progress);
        }

        int fill = (state == CreatorBlockEntity.STATE_READY) ? 24
                : (state == CreatorBlockEntity.STATE_PROCESSING ? progress * 24 / 100 : 0);
        if (fill > 0) {
            ctx.drawTexture(RenderLayer::getGuiTextured, BG, x + 79, y + 34, 176, 14, fill, 16, 256, 256);
        }
    ...
    private void drawProcessingEffect(DrawContext ctx, int progress) {
        int cx = x + 91;
        int cy = y + 42;
        for (int i = 0; i < 6; i++) {
            ...
            int colour = (alpha << 24) | 0xAA44FF;
            ctx.fill(px, py, px + 2, py + 2, colour);
        }
        float pulse = (float) Math.abs(Math.sin(animTimer));
        int glowAlpha = (int)(80 * pulse);
        ctx.fill(x + 79, y + 34, x + 79 + 24, y + 34 + 16,
                (glowAlpha << 24) | 0xAA44FF);
    }
    ...
    private void drawOutputSprite(DrawContext ctx, int state) {
        ...
        ctx.drawTexture(RenderLayer::getGuiTextured, texId, x + 134, y + 35, 0, 0, 16, 16, 16, 16);
    }
}