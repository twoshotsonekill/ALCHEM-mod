...
public class ItemAbilityEvents {
    ...
    public static void register() {
        UseItemCallback.EVENT.register((player, world, hand) -> {
            ...
            // Accept both the new OddityItem and legacy DynamicItem stacks.
            // The only thing that matters is presence of creator_special in NBT.
            if (!(stack.getItem() instanceof DynamicItem) && !(stack.getItem() instanceof OddityItem)) {
                return ActionResult.PASS;
            }

            NbtComponent customData = stack.get(DataComponentTypes.CUSTOM_DATA);
            if (customData == null) return ActionResult.PASS;

            NbtCompound tag = customData.copyNbt();

            // If the item has a behavior script it handles everything itself via
            // DynamicItem.use() / OddityItem.use() — don't double-fire here.
            if (!tag.getString("creator_script").isBlank()) {
                return ActionResult.PASS;
            }

            // Read special directly from NBT — no registry lookup, survives restarts.
            String special = tag.getString("creator_special");
            if (special.isBlank()) return ActionResult.PASS;

            if (world.isClient) return ActionResult.SUCCESS;

            ServerWorld serverWorld = (ServerWorld) world;
            boolean didAction = switch (special) {
                case "launch" -> {
                ...
                case "lightning" -> {
                    Vec3d look = player.getRotationVec(1.0f).multiply(15);
                    Vec3d target = player.getPos().add(look);
